  ...
  describe('messages', () => {
    ...
    describe('未認証の場合', () => {
      ...
      it('書き込みできない', async () => {
        ...
        await assertFails(uploadBytes(newStorageRef, file, {
          customMetadata: { ownerId: other.id }
        }))
      })
    })
    ...
    describe('認証済みの場合', () => {
      ...
      describe('自分のファイルの場合', () => {
        ...
        it('書き込みできる', async () => {
          const newStorageRef = ref(storage, 'messages/new-message-id/sample.png')
          await assertSucceeds(uploadBytes(newStorageRef, file, {
            customMetadata: { ownerId: user.id }
          }))
        })
        ...
        it('変更できる', async () => {
          const storageRef = ref(storage, userFilePath)
          await assertSucceeds(uploadBytes(storageRef, file, {
            customMetadata: { ownerId: user.id }
          }))
        })
      })
      ...
      describe('他ユーザーのファイルの場合', () => {
        ...
        it('変更できない', async () => {
          const storageRef = ref(storage, otherFilePath)
          await assertFails(uploadBytes(storageRef, file, {
            customMetadata: { ownerId: other.id }
          }))
        })
      })
